...
$errors = array();
    ...
    $password = $_POST['password'];
		...
		if (strlen($password)<=8) {
            ...
            $result = mysqli_query($db, "SELECT ques,ans FROM userdetails WHERE email = '$email'");
            $retrieve = mysqli_fetch_array($result);
            $QUES = $retrieve['ques'];
            $ANS = $retrieve['ans'];
            if($ques == $QUES && $ans == $ANS){
            mysqli_query($db, "UPDATE userdetails SET password='$password'");
            echo "<script type='text/javascript'>alert('Password Changed Successfully')</script>";
            }else{
                array_push($errors, "Security Question/Answer is wrong");
						}
		}
    else{
        array_push($errors, "Email does not exits");
    }
	}
...
<p>&nbsp;&nbsp;&nbsp;	Please fill up below question to reset password !</p>

<center><form action="cust_forgotpass.php" method="POST">
<?php include('Error.php'); ?>
  <table>
   <tr>
    <td>Email* :</td>
    <td><input type="email" name="email" size="80"  value="<?php echo $email; ?>" required></td>
   </tr>
   <tr>
    <td><br></td>
    <td><br></td>
   ...
   <tr>
    <td>Answer* :</td>
    <td><input type="text" name="ans" size="80" value="<?php echo $ans; ?>" required></td>
   </tr>
   <tr>
    <td><br></td>
    <td><br></td>
   ...
   <tr>
    ...
    <td><input type="password" name="cpassword" size="80" required></td>
   </tr>
   <tr>
    <td></td>
	<td><center><input type="submit" value="Sumbit" name="sumbit"></center></td>
   </tr>
  </table>
 </form></center>
<br>
